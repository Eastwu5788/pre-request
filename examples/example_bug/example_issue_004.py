# !/usr/local/python/bin/python
# -*- coding: utf-8 -*-
# (C) Wu Dong, 2020
# All rights reserved
# @Author: 'Wu Dong <wudong@eastwu.cn>'
# @Time: '2020-12-21 13:15'
import json
from flask import make_response, Flask
from pre_request import pre, Rule


app = Flask(__name__)
app.config["TESTING"] = True
client = app.test_client()


def json_resp(result):
    result = json.dumps(result)
    resp = make_response(result)
    resp.headers['Content-Type'] = 'application/json'
    return resp


args = {
    "userFirst": {
        "userId": Rule(type=int, required=False),
        "socialInfo": {
            "gender": Rule(type=int, required=False, enum=[1, 2], default=1),
            "age": Rule(type=int, gte=18, lt=80, default=20, required=False),
            "country": Rule(required=False, default="CN", deep=False)
        }
    }
}


@app.route("/structure", methods=["GET", "POST"])
@pre.catch(args)
def structure_handler(params):
    return json_resp(params)


if __name__ == "__main__":
    params = {
        "userFirst": {
            "userId": 14,
        }
    }
    resp = app.test_client().post("/structure", json=params)
    print(resp.json)
